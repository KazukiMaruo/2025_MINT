...
download_datashare_dir(datashare_dir = DATASHARE_RAW_FOLDER,
                       target_dir = os.path.join({BASE_DIR}, 'data'), #  "BASE_DIR": "/u/kazma/MINT/",
                       datashare_user = DATASHARE_USER) # "DATASHARE_USER": "kazma",


# get eeg headers from datashare
# update_eeg_headers(f"{BASE_DIR}/data/raw/{session}/{session}.vhdr") # BrainVision file format commonly used for EEG data.a